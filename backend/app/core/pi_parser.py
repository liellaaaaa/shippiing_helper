...
def parse_proforma_invoice(rows: list[list[str]]) -> PiContractUploadResponse:
    ...
    # 目的港源数据规范化：中英混合取英文，其余保留原样（台账=源数据，报关时再转中文）
    if destination:
        destination = normalize_destination_source(destination)
...
def _build_response_from_ai(ai_data: dict) -> PiContractUploadResponse:
    ...
    return PiContractUploadResponse(
        ...
        destination=normalize_destination_source(ai_data.get("destination")),

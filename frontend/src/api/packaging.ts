...
const packagingApi = {
  ...
  getTypes(): Promise<PackageType[]> {
    return apiClient.get('/packaging/types').then(r => r.data)
  },

  /** 获取所有托盘种类 */
  getPallets(): Promise<PalletType[]> {
    return apiClient.get('/packaging/pallets').then(r => r.data)
  },

  /** 计算包装方案 */
  calculate(data: CalculateRequest): Promise<PackingScheme> {
    return apiClient.post('/packaging/calculate', data).then(r => r.data)
  },

  /** 计算所有可用方案 */
  calculateSchemes(data: CalculateRequest): Promise<PackingScheme[]> {
    return apiClient.post('/packaging/calculate-schemes', data).then(r => r.data)
  },
}

...
def _to_prefixed_no(contract_no: Optional[str], out_prefix: str) -> str:
    if not contract_no:
        return ""
...
def to_invoice_no(contract_no: Optional[str]) -> str:
    """HT260720SZ → IN260720SZ；IN 开头原样返回。"""
    return _to_prefixed_no(contract_no, "IN")


def to_packing_no(contract_no: Optional[str]) -> str:
